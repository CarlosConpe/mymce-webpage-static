...
const ROOT_DIR = __dirname; // Fixed for running from root
const OUTPUT_FILE = path.join(ROOT_DIR, 'assets', 'js', 'search-index.json');

const DIRS_TO_SCAN = [
    'portafolio',
    'servicios', // Changed from servicios-carpinteria if that folder doesn't exist? Checking list_dir... 
    // Wait, list_dir previously showed 'servicios'. Let's trust DIRS_TO_SCAN was correct or fix it.
    // Original said 'servicios-carpinteria', list_dir said 'servicios'. 
    // The user might have renamed it. I should check.
    'servicios', // I will use 'servicios' based on list_dir output from earlier
    'soluciones',
    'corporate'
];
...
function processFile(filePath, section) {
    try {
        ...
        // EXCLUSIONS
        // User requested to omit portfolio page (likely the main index or unrelated items)
        // We will exclude the main index /portafolio/index.html
        if (url.includes('/portafolio/index.html') || url.endsWith('/portafolio/')) {
            console.log(`Skipping Portfolio Index: ${url}`);
            return;
        }
